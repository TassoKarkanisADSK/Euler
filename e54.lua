...
local function cardValue(c)
    local v = string.sub(c, 1, 1)
    return Values[v]
end


local function cardSuit(c)
    return string.sub(c, 2, 2)
end


local function sortHand(h)
    local cmp = function(c1, c2)
...
local function findMultiple(h, mult)
    local v = handValues(h)
...
local function compareHighestCard(h1, h2)
    return cardValue(h1[5]) < cardValue(h2[5])
end

--
-- predicates for hand types
--

local function isStraight(h)
    for i = 1, 4 do
        if cardValue(h[i+1]) ~= cardValue(h[i]) + 1 then
            return false
        end
    end
...
local function isFullHouse(h)
    local v1 = cardValue(h[1])
    local v2 = cardValue(h[2])
    local v3 = cardValue(h[3])
    local v4 = cardValue(h[4])
    local v5 = cardValue(h[5])
...
local function isOnePair(h)
    local v = handValues(h)
    for val,mult in pairs(v) do
        if mult == 2 then
            return true
        end
    end
...
local function compareOnePair(h1, h2)
    local m1 = findMultiple(h1, 2)
    local m2 = findMultiple(h2, 2)
    if m1 < m2 then
        return true
    elseif m1 > m2 then
        return false
    else
        local v1 = cardValue(h1[5])
        local v2 = cardValue(h2[5])
        assert(v1 ~= v2)
        return v1 < v2
    end
end
...
local function isFlush(h)
    local s1 = cardSuit(h[1])
    local s2 = cardSuit(h[2])
    local s3 = cardSuit(h[3])
    local s4 = cardSuit(h[4])
    local s5 = cardSuit(h[5])

    if s1 == s2 and s2 == s3 and s3 == s4 and s4 == s5 then
        return true
    end
...
local function isStraightFlush(h)
    return isStraight(h) and isFlush(h)
end


local function isRoyalFlush(h)
    return isStraightFlush(h) and cardValue(h[1]) == 9
end


--
-- data to determine hand quality
--
...
local HandTypes = {
    {
        name = "royal flush",
        accept = isRoyalFlush,
        compare = compareHighestCard,
        value = 10,
    },
    {
        name = "straight flush",
        accept = isStraightFlush,
        compare = compareHighestCard,
        value = 9,
    },
    {
...
local function handType(h)
    for _,t in ipairs(HandTypes) do
        if t.accept(h) then
            return t
        end
    end
...
local function handGreater(h1, h2)
    local t1 = handType(h1)
    local t2 = handType(h2)
        
    -- print(table.concat(h1," "), t1.name)
    -- print(table.concat(h2," "), t2.name)

    local leftWins
    if t1.name ~= t2.name then
        leftWins = t1.value > t2.value
    else
        leftWins = not t1.compare(h1, h2)
    end
...
local function run()
    ...
    for i = 1, #data, 10 do
        total = total + 1
        local h1 = { data[i], data[i+1], data[i+2], data[i+3], data[i+4] }
        local h2 = { data[i+5], data[i+6], data[i+7], data[i+8], data[i+9] }
        
        sortHand(h1)
        sortHand(h2)

        local greater = handGreater(h1, h2)
        if greater then
            count = count + 1
        end
    end
